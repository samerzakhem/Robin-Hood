...
nfuzion.application.Application = function(name) {
	...
	haxe.Log.trace("Built: " + "2013-12-06 15:43:14",{ fileName : "Application.hx", lineNumber : 54, className : "nfuzion.application.Application", methodName : "new"});
};
...
var AppMain = function() {
	nfuzion.nTactic.NTactic.call(this,"nTester");
	this.assignAssetsPath("../assets/");
	nfuzion.nTactic.NTactic.builder.set_sketch(new nfuzion.sketch.XmlSketch("assets.xml"));
};
$hxClasses["AppMain"] = AppMain;
AppMain.__name__ = ["AppMain"];
AppMain.__super__ = nfuzion.nTactic.NTactic;
AppMain.prototype = $extend(nfuzion.nTactic.NTactic.prototype,{
	start: function() {
...
nfuzion.message.amp = {}
nfuzion.message.amp.LetSource = function(value) {
	nfuzion.message.generic.templates.LetInt32.call(this,value);
};
$hxClasses["nfuzion.message.amp.LetSource"] = nfuzion.message.amp.LetSource;
nfuzion.message.amp.LetSource.__name__ = ["nfuzion","message","amp","LetSource"];
nfuzion.message.amp.LetSource.__super__ = nfuzion.message.generic.templates.LetInt32;
nfuzion.message.amp.LetSource.prototype = $extend(nfuzion.message.generic.templates.LetInt32.prototype,{
	__class__: nfuzion.message.amp.LetSource
});
...
peripheral.Peripheral.initialize = function() {
	nfuzion.nTactic.NTactic.cacheManager.set_cacheAll(true);
	...
	try {
		peripheral.Peripheral.spanClient = new nfuzion.span.SpanClient(peripheral.Peripheral.urlRecord.value,null,"Message Sender","Development");
	} catch( e ) {
		peripheral.Peripheral.spanClient = null;
		haxe.Log.trace("NOTICE: Invalid url. Using default.",{ fileName : "Peripheral.hx", lineNumber : 40, className : "peripheral.Peripheral", methodName : "initialize"});
...
peripheral.Peripheral.useDefault = function() {
	peripheral.Peripheral.urlRecord.set_value("ws://localhost:4410");
}
...
var screen = {}
screen.BaseScreen = function(name) {
	nfuzion.nTactic.core.DynamicScreen.call(this,name);
};
$hxClasses["screen.BaseScreen"] = screen.BaseScreen;
screen.BaseScreen.__name__ = ["screen","BaseScreen"];
screen.BaseScreen.__super__ = nfuzion.nTactic.core.DynamicScreen;
screen.BaseScreen.prototype = $extend(nfuzion.nTactic.core.DynamicScreen.prototype,{
	onButton: function(e) {
		var name = e.target.name;
		name = name.substring(0,name.indexOf("_"));
		haxe.Log.trace("Button Clicked: " + name,{ fileName : "BaseScreen.hx", lineNumber : 169, className : "screen.BaseScreen", methodName : "onButton"});
		if(StringTools.startsWith(name,"local")) {
			var screen = this.screens.get(name);
			if(screen != null) nfuzion.nTactic.NTactic.screens["goto"](screen);
		} else {
			var message = this.messages.get(name);
			if(message != null) peripheral.Peripheral.spanClient.send(message); else haxe.Log.trace("WARNING: Message " + name + " not found.",{ fileName : "BaseScreen.hx", lineNumber : 187, className : "screen.BaseScreen", methodName : "onButton"});
		}
	}
	,getY: function() {
		var rowIndex = Math.floor(this.buttonCount / this.buttonsPerRow);
		return rowIndex * 50;
	}
	,getX: function() {
		var columnIndex = this.buttonCount % this.buttonsPerRow;
		return columnIndex * 160;
	}
	,createButton: function(foundation,name,font,staticText,width,height,x,y,borderPaint) {
		var buttonSketch = new nfuzion.sketch.type.SketchContainer(name,x,y,width,height);
		buttonSketch.borderPaint = borderPaint;
		buttonSketch.borderWidth = 3;
		var textSketch = null;
		if(font != null) {
			textSketch = new nfuzion.sketch.type.SketchText("text_label",2,10,width - 4,height - 12);
			textSketch.paint = this.buttonTextPaint;
		...
		downGuise.backgroundPaint = this.buttonDownPaint;
		downGuise.borderPaint = borderPaint;
		downGuise.borderWidth = 3;
	...
	,addButton: function(title,message) {
		var name = Std.string(this.getX() + "-" + this.getY());
		var button = this.createButton(this,name + "_button",this.primaryFont,title,160,50,this.getX(),this.getY(),this.buttonBorderPaint);
		this.buttonArray.push(name + "_button");
		this.messages.set(name,message);
		message;
		this.buttonCount++;
	}
	,addLocalButton: function(title,screenName) {
		var name = "local" + Std.string(this.getX() + "-" + this.getY());
		var button = this.createButton(this,name + "_button",this.primaryFont,title,160,50,this.getX(),this.getY(),this.localButtonPaint);
		this.buttonArray.push(name + "_button");
		this.screens.set(name,screenName);
		screenName;
		this.buttonCount++;
	}
	,addButtons: function() {
		this.addLocalButton("< Back","Home");
	}
	,layoutScreen: function() {
		nfuzion.nTactic.core.DynamicScreen.prototype.layoutScreen.call(this);
		this.messages = new haxe.ds.StringMap();
		this.screens = new haxe.ds.StringMap();
		this.buttonCount = 0;
		this.buttonsPerRow = Math.floor(nfuzion.nTactic.NTactic.stage._width / 160);
		this.buttonArray = new Array();
		var bg = this.createContainer("background_simple",nfuzion.nTactic.NTactic.stage._width | 0,nfuzion.nTactic.NTactic.stage._height | 0);
	...
	,defineFonts: function() {
		...
		this.primaryFont = this.setFont("statsoverlayprimary",this.formatPath("./fonts/GillSansLight.ttf"),16);
	}
	,definePaints: function() {
		nfuzion.nTactic.core.DynamicScreen.prototype.definePaints.call(this);
		this.bgPaint = this.setPaint("background","0x222222");
		this.buttonBorderPaint = this.setPaint("border","0x78C043");
		this.localButtonPaint = this.setPaint("local","0xFF7F2A");
		this.buttonTextPaint = this.setPaint("text","0xffffff");
		this.buttonDownPaint = this.setPaint("down","0x555555");
	}
	,addListeners: function() {
		nfuzion.nTactic.core.DynamicScreen.prototype.addListeners.call(this);
		var _g = 0, _g1 = this.buttonArray;
		while(_g < _g1.length) {
			var name = _g1[_g];
			++_g;
			var widget = this.getWidget(name);
			this.attachListener(widget,"ButtonEvent.click",$bind(this,this.onButton));
		}
	}
	,setupWidgets: function() {
	...
	,screens: null
	,messages: null
	,buttonTextPaint: null
	,buttonDownPaint: null
	,localButtonPaint: null
	,buttonBorderPaint: null
	,bgPaint: null
	,primaryFont: null
	,__class__: screen.BaseScreen
});
screen.HmiClusterTestScreen = function(name) {
	screen.BaseScreen.call(this,name);
};
$hxClasses["screen.HmiClusterTestScreen"] = screen.HmiClusterTestScreen;
screen.HmiClusterTestScreen.__name__ = ["screen","HmiClusterTestScreen"];
screen.HmiClusterTestScreen.__super__ = screen.BaseScreen;
screen.HmiClusterTestScreen.prototype = $extend(screen.BaseScreen.prototype,{
	addButtons: function() {
		screen.BaseScreen.prototype.addButtons.call(this);
		this.addButton("Ignition On",new nfuzion.message.vehicle.LetStarted(true));
		this.addButton("Ignition Off ",new nfuzion.message.vehicle.LetStarted(false));
		this.addButton("ABS - T",new nfuzion.message.vehicle.LetABS(true));
		this.addButton("ABS - F",new nfuzion.message.vehicle.LetABS(false));
		this.addButton("Airbag - T",new nfuzion.message.vehicle.LetAirbag(true));
		this.addButton("Airbag - F",new nfuzion.message.vehicle.LetAirbag(false));
		this.addButton("Battery - T",new nfuzion.message.vehicle.LetBattery(true));
		this.addButton("Battery - F",new nfuzion.message.vehicle.LetBattery(false));
		this.addButton("E-Brake - T",new nfuzion.message.vehicle.LetEmergencyBrake(true));
		this.addButton("E-Brake - F",new nfuzion.message.vehicle.LetEmergencyBrake(false));
		this.addButton("HiBeam - T",new nfuzion.message.vehicle.LetHighBeam(true));
		this.addButton("HiBeam - F",new nfuzion.message.vehicle.LetHighBeam(false));
		this.addButton("Odometer A",new nfuzion.message.vehicle.LetOdometer(8675309.1));
		this.addButton("Odometer B",new nfuzion.message.vehicle.LetOdometer(59368.7));
		this.addButton("Oil - T",new nfuzion.message.vehicle.LetOil(true));
		this.addButton("Oil - F",new nfuzion.message.vehicle.LetOil(false));
		this.addButton("Seatbelt - T",new nfuzion.message.vehicle.LetSeatBelt(true));
		this.addButton("Seatbelt - F",new nfuzion.message.vehicle.LetSeatBelt(false));
		this.addButton("TracCtrl - T",new nfuzion.message.vehicle.LetTractionControl(true));
		this.addButton("TracCtrl - F",new nfuzion.message.vehicle.LetTractionControl(false));
		this.addButton("Fuel 33%",new nfuzion.message.vehicle.LetFuel(0.33));
		this.addButton("Fuel 100%",new nfuzion.message.vehicle.LetFuel(1));
		this.addButton("Fuel 0%",new nfuzion.message.vehicle.LetFuel(0));
		this.addButton("Dis2E 777.7",new nfuzion.message.vehicle.LetDistanceToEmpty(777.7));
		this.addButton("Dis2E 207",new nfuzion.message.vehicle.LetDistanceToEmpty(20));
		this.addButton("Speed - 55",new nfuzion.message.vehicle.LetSpeed(55.1));
		this.addButton("Speed - 106",new nfuzion.message.vehicle.LetSpeed(106));
		this.addButton("Speed - 8",new nfuzion.message.vehicle.LetSpeed(8));
		this.addButton("PRND - P",new nfuzion.message.vehicle.LetTransmission(nfuzion.message.vehicle.type.TransmissionState.park));
		this.addButton("PRND - R",new nfuzion.message.vehicle.LetTransmission(nfuzion.message.vehicle.type.TransmissionState.reverse));
		this.addButton("PRND - N",new nfuzion.message.vehicle.LetTransmission(nfuzion.message.vehicle.type.TransmissionState.nuetral));
		this.addButton("PRND - D",new nfuzion.message.vehicle.LetTransmission(nfuzion.message.vehicle.type.TransmissionState.drive));
		this.addButton("TurnSig - L",new nfuzion.message.vehicle.LetTurnSignal(nfuzion.message.vehicle.type.TurnSignalState.left));
		this.addButton("TurnSig - R",new nfuzion.message.vehicle.LetTurnSignal(nfuzion.message.vehicle.type.TurnSignalState.right));
		this.addButton("TurnSig - N",new nfuzion.message.vehicle.LetTurnSignal(nfuzion.message.vehicle.type.TurnSignalState.none));
		this.addButton("TurnSig - B",new nfuzion.message.vehicle.LetTurnSignal(nfuzion.message.vehicle.type.TurnSignalState.both));
		this.addButton("WaterTemp - 77",new nfuzion.message.vehicle.LetWaterTemperature(.77));
		this.addButton("WaterTemp - 15",new nfuzion.message.vehicle.LetWaterTemperature(.15));
		this.addButton("Nav - BearLeft",new nfuzion.message.navigation.LetNextTurn(new nfuzion.message.navigation.type.StepData(nfuzion.message.navigation.type.TargetType.bearLeft,1941.0,"Pearl Harbor Way")));
		this.addButton("Nav - BearRight",new nfuzion.message.navigation.LetNextTurn(new nfuzion.message.navigation.type.StepData(nfuzion.message.navigation.type.TargetType.bearRight,2001.0,"Trade Plaza")));
		this.addButton("Nav - Left",new nfuzion.message.navigation.LetNextTurn(new nfuzion.message.navigation.type.StepData(nfuzion.message.navigation.type.TargetType.left,1776.0,"Lexington Hwy")));
		this.addButton("Nav - Right",new nfuzion.message.navigation.LetNextTurn(new nfuzion.message.navigation.type.StepData(nfuzion.message.navigation.type.TargetType.right,1492.0,"Santo Domingo St")));
		this.addButton("Nav - UTurn",new nfuzion.message.navigation.LetNextTurn(new nfuzion.message.navigation.type.StepData(nfuzion.message.navigation.type.TargetType.uturn,1066.0,"Normandy Lane")));
		this.addButton("Nav Dist 620",new nfuzion.message.navigation.LetDistance(620));
		this.addButton("Nav Dist 2585",new nfuzion.message.navigation.LetDistance(2585));
		this.addButton("Nav Dist 605548",new nfuzion.message.navigation.LetDistance(605548));
	}
	,__class__: screen.HmiClusterTestScreen
});
screen.Home = function(name) {
	screen.BaseScreen.call(this,name);
};
$hxClasses["screen.Home"] = screen.Home;
screen.Home.__name__ = ["screen","Home"];
screen.Home.__super__ = screen.BaseScreen;
screen.Home.prototype = $extend(screen.BaseScreen.prototype,{
	addButtons: function() {
		this.addLocalButton("Span Config","popup:SpanConfigPopup");
		this.addLocalButton("nService","NServiceTestScreen");
		this.addLocalButton("Robin Hood Cluster","HmiClusterTestScreen");
		this.addLocalButton("Robin Hood HMI","RobinhoodHmi");
	}
	,__class__: screen.Home
});
screen.NServiceTestScreen = function(name) {
	screen.BaseScreen.call(this,name);
};
$hxClasses["screen.NServiceTestScreen"] = screen.NServiceTestScreen;
screen.NServiceTestScreen.__name__ = ["screen","NServiceTestScreen"];
screen.NServiceTestScreen.__super__ = screen.BaseScreen;
screen.NServiceTestScreen.prototype = $extend(screen.BaseScreen.prototype,{
	addButtons: function() {
		screen.BaseScreen.prototype.addButtons.call(this);
		this.addButton("Chime Test 3x",new nfuzion.message.chime.SetChime(nfuzion.message.chime.type.Chime.warning,3));
		this.addButton("VR Start",new nfuzion.message.vr.SetCapture(true));
		this.addButton("VR Stop",new nfuzion.message.vr.SetCapture(false));
		this.addButton("TTS Speak Text",new nfuzion.message.tts.SetText("Everything is Ok"));
		this.addButton("TTS Cache Text",new nfuzion.message.tts.SetCacheText("Everything is going to be Ok"));
		this.addButton("Add Notice A",new nfuzion.message.notice.SetAddNotice(new nfuzion.message.notice.type.Notice("Check it out","I cant believe it!",nfuzion.message.notice.type.NoticeType.waypointSuggestion,nfuzion.message.notice.type.NoticePriority.low)));
		this.addButton("Add Notice B",new nfuzion.message.notice.SetAddNotice(new nfuzion.message.notice.type.Notice("Uh oh","You forgot your wallet!",nfuzion.message.notice.type.NoticeType.socialMedia,nfuzion.message.notice.type.NoticePriority.high)));
		this.addButton("Add Notice C",new nfuzion.message.notice.SetAddNotice(new nfuzion.message.notice.type.Notice("Note","Did you notice this notice?",nfuzion.message.notice.type.NoticeType.text,nfuzion.message.notice.type.NoticePriority.low)));
		this.addButton("Play Media",new nfuzion.message.media.SetTransportAction(nfuzion.message.media.type.TransportAction.play));
		this.addButton("Pause Media",new nfuzion.message.media.SetTransportAction(nfuzion.message.media.type.TransportAction.pause));
		this.addButton("Next Track",new nfuzion.message.media.SetTransportAction(nfuzion.message.media.type.TransportAction.next));
		this.addButton("Previous Track",new nfuzion.message.media.SetTransportAction(nfuzion.message.media.type.TransportAction.previous));
		this.addButton("Pandora GetStatus",new nfuzion.message.pandora.GetStatus());
		this.addButton("Pandora GetChannels",new nfuzion.message.pandora.GetChannels(0,2));
		this.addButton("Pandora SetChannel 0",new nfuzion.message.pandora.SetChannel(1));
		this.addButton("Pandora SetChannel 1",new nfuzion.message.pandora.SetChannel(1));
		this.addButton("Pandora SetChannel 2",new nfuzion.message.pandora.SetChannel(2));
		this.addButton("Pandora getCurrentChannel",new nfuzion.message.pandora.GetCurrentChannel());
		this.addButton("1.ClearRoute",new nfuzion.message.navigation.SetClearRoute());
		this.addButton("2.SetStartPoint",new nfuzion.message.navigation.SetStartByPoint(42.47727,-83.09807421395362));
		this.addButton("3.SetEndPoint",new nfuzion.message.navigation.SetEndByPoint(42.258252,-82.9453683));
		this.addButton("4.requestNewRoute",new nfuzion.message.navigation.SetRequestNewRoute(nfuzion.message.navigation.type.TransitType.driving));
		this.addButton("GetRoute",new nfuzion.message.navigation.GetRoute());
		this.addButton("GetDistance",new nfuzion.message.navigation.GetDistance());
		this.addButton("GetDistance %",new nfuzion.message.navigation.GetDistancePercentage());
		this.addButton("GetNextTurn",new nfuzion.message.navigation.GetNextTurn());
		this.addButton("GetWaypoints",new nfuzion.message.navigation.GetWaypoints());
		this.addButton("Hands Free On",new nfuzion.message.phone.LetHandsFree(true));
		this.addButton("Hands Free Off",new nfuzion.message.phone.LetHandsFree(false));
		this.addButton("AmpSource Phone",new nfuzion.message.amp.LetSource(3));
		this.addButton("AmpSource Media",new nfuzion.message.amp.LetSource(0));
	}
	,__class__: screen.NServiceTestScreen
});
screen.RobinhoodHmi = function(name) {
	screen.BaseScreen.call(this,name);
};
$hxClasses["screen.RobinhoodHmi"] = screen.RobinhoodHmi;
screen.RobinhoodHmi.__name__ = ["screen","RobinhoodHmi"];
screen.RobinhoodHmi.__super__ = screen.BaseScreen;
screen.RobinhoodHmi.prototype = $extend(screen.BaseScreen.prototype,{
	addButtons: function() {
		...
		this.addButton("Call Duration 35",new nfuzion.message.phone.LetCallDuration(35));
		this.addButton("Call Duration 489",new nfuzion.message.phone.LetCallDuration(489));
		this.addButton("InCall Existing Contact",new nfuzion.message.phone.LetStatus(nfuzion.message.phone.type.PhoneStatus.inCall,"248-930-4956"));
		this.addButton("InCall: No Contact",new nfuzion.message.phone.LetStatus(nfuzion.message.phone.type.PhoneStatus.inCall,"900-555-1212"));
		this.addButton("PhoneStatus: Idle",new nfuzion.message.phone.LetStatus(nfuzion.message.phone.type.PhoneStatus.idle));
		this.addButton("Next Destination",new nfuzion.message.notice.SetAddNotice(new nfuzion.message.notice.type.Notice("Robinhood Meeting","Meet Robinhood Team",nfuzion.message.notice.type.NoticeType.destination,nfuzion.message.notice.type.NoticePriority.high,[new nfuzion.message.notice.type.Action("Set as Destination",nfuzion.message.notice.type.ActionType.setDestination,"42.258252,-82.9453683"),new nfuzion.message.notice.type.Action("Dismiss",nfuzion.message.notice.type.ActionType["delete"])],new nfuzion.message.notice.type.EventData("Photoshoot","Rose Gardens","2:45","Anne Geddis","555-555-5555"))));
		this.addButton("Waypoint Suggestion",new nfuzion.message.notice.SetAddNotice(new nfuzion.message.notice.type.Notice("Calender Change","You have some extra time. Would you like to stop for Coffee?",nfuzion.message.notice.type.NoticeType.waypointSuggestion,nfuzion.message.notice.type.NoticePriority.medium,[new nfuzion.message.notice.type.Action("Add to Route",nfuzion.message.notice.type.ActionType.addWaypoint,"42.560054,-83.183888"),new nfuzion.message.notice.type.Action("Dismiss",nfuzion.message.notice.type.ActionType["delete"])],new nfuzion.message.notice.type.EventData("Coffee","Starbucks",null,null,null))));
	}
	,__class__: screen.RobinhoodHmi
});
...
screen.SpanConfigPopup.prototype = $extend(nfuzion.nTactic.core.DynamicScreen.prototype,{
	...
	,layoutScreen: function() {
		nfuzion.nTactic.core.DynamicScreen.prototype.layoutScreen.call(this);
		var screenWidth = nfuzion.nTactic.NTactic.stage._width | 0;
		var screenHeight = nfuzion.nTactic.NTactic.stage._height | 0;
		var screenRatio = .8;
...
screen.BaseScreen.BUTTON_WIDTH = 160;
screen.BaseScreen.BUTTON_HEIGHT = 50;
screen.BaseScreen.SHOW_GRID = false;
